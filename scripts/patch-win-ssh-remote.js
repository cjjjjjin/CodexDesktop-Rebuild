...
const MARKER = "try{s.terminate?.()}catch{}";
...
function applyWindowsSshRemoteGuardPatch(source) {
  ...
  if (patched.includes("openCodexWindowsSshWebSocket")) {
    patched = patched
      ...
      .replace("o=c,s.terminate?.();if(Date.now()>=a)", "o=c;try{s.terminate?.()}catch{}if(Date.now()>=a)");
